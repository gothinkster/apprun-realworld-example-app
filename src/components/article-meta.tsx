...
export default
  ...
  return <div class="article-meta">
    ...
    {app['user'] && app['user'].username === article.author.username
      ...
      : <span>
        <button class={`btn btn-sm ${followClass}`}
          onclick={e => app.run('/toggle-follow', article.author, component)}>
          {article.author.following
            ? <span><i class="ion-minus-round"></i> Unfollow {article.author.username}</span>
            : <span><i class="ion-plus-round"></i> Follow {article.author.username}</span>
          }
        </button> &nbsp;&nbsp;
        <button class={`btn btn-sm ${favClass}`}
